...
#values are assigned with respect to their position
...
x1[seq(1,10,2)]
x1
x1[-5]#eliminate fifth element
x1[-c(1,3,7)]#eliminate first,third & seventh element from the vector

#sort
sort(x1)
sort(x1, decreasing=T)#descending, =T means flagging it as true
x1
...
length(x3)
rep(1,5);rep(c(1,3),5)#rep(1,times=5)
ls()#variables used till now
rep(c(1,3),each=5)#all data together in clusters instead of one by one
(x4 = c(rep(4,times=3), rep(7, times=4)))
(x5 = c(rep(c(1,2,3), times=c(5,4,3))))
rep(1,5);rep(c(1,3),times=c(2,5))
...
sd(x6)
plot(density(x6))
abline(v=5,h=0.3,col=3:7)#col signifies color
